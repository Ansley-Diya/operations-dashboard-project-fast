...
function isPrimitiveArrayProxy(value) {
    return (value === null || value === void 0 ? void 0 : value[primitiveArrayProxyMarker]) === true;
}
function getPrimitiveArrayProxyTarget(value) {
...
function isReachableFromRoot(root, value) {
    if (root === value) {
        return true;
    }
...
function addReachableArray(registration, reachableArray) {
    if (!registration.reachableArrays.includes(reachableArray)) {
        registration.reachableArrays.push(reachableArray);
    }
}
function refreshArrayRegistrationReachability(registration, subject) {
    const roots = getArrayRegistrationRoots(registration);
    registration.reachableArrays = registration.reachableArrays.filter(reachableArray => roots.some(root => isReachableFromRoot(root, reachableArray)));
    return (roots.some(root => isReachableFromRoot(root, subject)) ||
        registration.reachableArrays.length > 0);
}
function getArrayRegistrationRoots(registration) {
    var _a;
    const ownerTargets = getTargetsForObject(registration.target);
    if (ownerTargets.length > 0) {
        return ownerTargets.map(targetItem => { var _a; return (_a = targetItem.target) === null || _a === void 0 ? void 0 : _a[targetItem.rootProperty]; });
    }
    return [(_a = registration.target) === null || _a === void 0 ? void 0 : _a[registration.rootProperty]];
}
function notifyArrayRegistration(registration) {
    Observable.notify(registration.target, registration.rootProperty);
    getTargetsForObject(registration.target).forEach((targetItem) => {
        Observable.notify(targetItem.target, targetItem.rootProperty);
    });
}
function isSameArrayObserverContext(registration, target, rootProperty, schema, rootSchema) {
...
function handleArrayChange(registration, subject, args) {
        ...
        if (arg.addedCount > 0) {
            if (schemaProperties) {
                for (let i = arg.addedCount - 1; i >= 0; i--) {
                    ...
                    assignProxyToItemsInArray(item, originalItem, registration.schema, registration.rootSchema, registration.target, registration.rootProperty);
...
function removeArrayRegistration(data, registration) {
...
function observeArray(data, reachableArray, schema, rootSchema, target, rootProperty) {
    const notifier = Observable.getNotifier(data);
    const observedArray = notifier.subject;
    let registrations = observedArraysMap.get(observedArray);
    if (!registrations) {
        registrations = [];
        observedArraysMap.set(observedArray, registrations);
    }
    const existingRegistration = registrations.find(registration => isSameArrayObserverContext(registration, target, rootProperty, schema, rootSchema));
...
function assignObservablesToArray(proxiedData, schema, rootSchema, target, rootProperty) {
    ...
    if (isPrimitiveArrayProxy(data)) {
        ...
        observeArray(primitiveArrayTarget, data, schema, rootSchema, target, rootProperty);
        return data;
    }
...
function addTargetToObject(object, target, rootProperty) {
    ...
    if (!targets.some(targetItem => targetItem.target === target && targetItem.rootProperty === rootProperty)) {
        ...
        const proxy = new Proxy(object, {
            set: (obj, prop, value) => {
                ...
                obj[prop] = hasObservableAccessor(obj, propName)
                    ? value
                    : assignObservables(childSchema !== null && childSchema !== void 0 ? childSchema : schema, rootSchema, value, target, rootProperty);
